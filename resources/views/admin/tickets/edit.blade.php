...
@section('post-scripts')
    ...
    <script>
        ...
        let attorneyId = {{ old('attorney_id', $ticket->attorney_id) }}; // Set the selected attorney ID
        ...
        var attorneysChoices = new Choices(attorneys, {
            ...
            maxItemCount: 5,
            shouldSort: false,  // Prevent sorting so selection order is preserved
            itemSelectText: '', // Optional, removes the 'Press to select' label
        });

        // Fetch and populate choices
        fetch('{{ route('api.attorney.index') }}')
            .then(response => response.json())
            .then(data => {
                // Map fetched data to Choices.js option format
                let options = data.map(attorney => ({
                    value: attorney.roleable.id.toString(),
                    label: attorney.name,
                    selected: attorney.roleable.id === attorneyId, // Set selected for matching ID
                    customProperties: {
                        officeHours: (attorney.roleable.office_hours_start ?? '') + ' - ' + (attorney.roleable.office_hours_end ?? ''),
                        attorneyPhone: attorney.phone ?? '',
                        attorneyAddress: attorney.address ?? '',
                    }
                }));

                // Set choices in Choices.js with reset to overwrite initial state
                attorneysChoices.setChoices(options, 'value', 'label', true);

                // Ensure correct option is selected programmatically after choices load
                if (attorneyId) {
                    console.log(attorneyId);
                    attorneysChoices.setChoiceByValue(attorneyId.toString());
                }
            })
            .catch(error => console.error('Error loading attorneys:', error));

        // Event listener to populate fields based on selected option
        attorneys.addEventListener('choice', function(event) {
            document.querySelector('#attorneyOfficeHours').value = event.detail.choice.customProperties.officeHours ?? '';
            document.querySelector('#attorneyPhone').value = event.detail.choice.customProperties.attorneyPhone ?? '';
            document.querySelector('#attorneyAddress').value = event.detail.choice.customProperties.attorneyAddress ?? '';
        });
                    ...
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 201) {
                            ...
                            document.getElementById('notesList').innerHTML += ('<div class="col-span-12 md:col-span-6">'+
                                ...
                                '<span class="text-muted text-sm float-end"> {{ \Illuminate\Support\Facades\Auth::user()->name }}</span>'+
                                '</div>'+
                                '</div>'+
                                '</div>')
                            Toast.fire({
